-- ============================================================
-- miiCase — human-readable names for synced records
-- 0016_record_description.sql
-- ============================================================
-- ModMed attachments ship no title, so synced filenames are machine ids
-- ("other-file|257403748.bin") — unusable as an identifier in the dashboard.
-- The DocumentReference *description* is human text and already lands in
-- emr_record_staging; carry it onto records, backfill what's synced, and the
-- dashboard shows description over filename.
-- ============================================================

alter table records add column if not exists description text;

-- Same body as 0015 plus description on insert/update.
...
-- Backfill already-synced records from the freshest staged description.
update records r set description = s.description
from (
  select distinct on (emr_document_id) emr_document_id, description
  from emr_record_staging
  where description is not null and description <> ''
  order by emr_document_id, created_at desc
) s
where r.emr_document_id = s.emr_document_id
